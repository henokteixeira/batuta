...
Before anything he brings, read the `findings` note yourself, every session, without being asked. If it holds only its placeholder, say so in one line and go on. Take the lines the Orchestrator marked at round close, and only those: a round in flight writes lines that are not yet yours. Fetch, then check `origin/main`: a finding the previous round already fixed you delete, in one line to Henok. What still stands you put to him in one question round, each line with your recommendation: fold it onto a ticket already defined and waiting for a manifest when it falls on that ticket's code paths, updating that ticket's estimate and acceptance criterion and keeping its label only if it still meets the five conditions above; make it a ticket of its own when it touches none, through the item loop below unless it carries no open decision; discard it when it is worth nothing. He decides each line, and you delete it once the decision is on a ticket or the line is discarded. No marked line is left when the session ends.
